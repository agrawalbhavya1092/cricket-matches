    ...
    "undefined" != typeof ko && ko.bindingHandlers && !ko.bindingHandlers.multiselect && (ko.bindingHandlers.multiselect = {
        ...
        defaults: {
            ...
            templates: {
                ...
                ul: '<ul class="multiselect-container dropdown-menu" style="width:11rem"></ul>',
                filter: '<li class="multiselect-item filter"><div class="input-group"><span class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span><input class="form-control multiselect-search" type="text"></div></li>',
                filterClearBtn: '<span class="input-group-btn"><button class="btn btn-default multiselect-clear-filter" type="button"><i class="glyphicon glyphicon-remove-circle"></i></button></span>',
                li: '<li style="margin-left:0.5rem"><div class="custom-control custom-checkbox my-1 mr-sm-2"><label></label></div></li>',
        ...
        buildDropdownOptions: function() {
            ...
            }, this)), t("li.multiselect-all", this.$ul).css("background", "#f3f3f3").css("border-bottom", "1px solid #eaeaea"), t("li.multiselect-group > div.custom-control, li.multiselect-all > div.custom-control > label.custom-control-label", this.$ul).css("padding", "3px 20px 3px 35px"), t("li.multiselect-group > div.custom-control > input", this.$ul).css("margin", "4px 0px 5px -20px"))
        ...
        createOptionValue: function(e) {
            ...
            var s = this.options.optionLabel(e),
                ...
                p = t("label", a);
            p.attr("for",s),p.addClass("custom-control-label"), a.addClass(l), this.options.enableHTML ? p.html(" " + s) : p.text(" " + s);
            // p.addClass(n), a.addClass(l), this.options.enableHTML ? p.html(" " + s) : p.text(" " + s);
            // p.addClass("custom-control-label")
            var h = t("<input/>").attr("type", n).attr('class','custom-control-input').attr('id',s);
            // this.options.checkboxName && h.attr("name", this.options.checkboxName), p.prepend(h);
            this.options.checkboxName && h.attr("name", this.options.checkboxName), p.before(h);
            var c = i.prop("selected") || !1;
            h.val(o), o === this.options.selectAllValue && (a.addClass("multiselect-item multiselect-all"), h.parent().parent().addClass("multiselect-all")), p.attr("title", i.attr("title")), this.$ul.append(a), i.is(":disabled") && h.attr("disabled", "disabled").prop("disabled", !0).closest("div").attr("tabindex", "-1").closest("li").addClass("disabled"), h.prop("checked", c), c && this.options.selectedClass && h.closest("li").addClass(this.options.selectedClass)
        },
        ...
        createOptgroup: function(e) {
            if (this.options.enableCollapsibleOptGroups && this.options.multiple) {
                var i = t(e).attr("label"),
                    s = t(e).attr("value"),
                    l = t('<li class="multiselect-item multiselect-group"><div class="custom-control custom-checkbox my-1 mr-sm-2"><input type="checkbox" value="' + s + '"/><b> ' + i + '<b class="caret"></b></b></a></li>');
        ...
        select: function(e, i) {
            ...
            this.createOptionTag(), this.updateSelectAll()
        },
        clearSelection: function() {
            console.log("clearSelection.......")
            this.deselectAll(!1), this.updateSelectAll()
        },
        ...
        rebuild: function() {
            console.log("rebuild...")
            this.$ul.html(""), this.options.multiple = "multiple" === this.$select.attr("multiple"), this.buildSelectAll(), this.buildDropdownOptions(), this.buildFilter(), this.updateSelectAll(!0), this.options.disableIfEmpty && t("option", this.$select).length <= 0 ? this.disable() : this.enable(), this.options.dropRight && this.$ul.addClass("pull-right")
        },
        ...
        createOptionTag: function() {
            var e = this.getSelected();
            console.log("get seldcted////",e)
            console.log("dddddddd",this.options.buttonText(e, this.$select))
            // $("#option_tags").html(this.options.buttonText(e, this.$select))
        
            // var name =  this.options.buttonText(e, this.$select)
            // var value =  this.options.buttonText(e, this.$select)
            // console.log("name.......",name)
            // var select_type = this.$select.siblings()[0].children[0].title
            // var select_type_id = this.$select.attr('id')
            // console.log("select_type...",select_type)
            // for(var i=0;i<name.length;i++){
            // if($("span[id='"+value[i]+"']").length == 0 && value[i] != select_type){
            //   var sp = $("<span></span>").attr("class","tag");
            //   var cross = $("<a></a>").attr("href","javascript:void(0)").attr("onclick","unselect(this)").text("x")
            //   var nametag = $("<span></span>").attr("id",value[i]).attr("title","select_type_id").html(name[i]+"&nbsp;&nbsp;")
            //   var tag = sp.append(nametag)
            //   tag = tag.append(cross)
        ...
        setAllSelectedText: function(t) {
            console.log("setAllSelectedText.........")
